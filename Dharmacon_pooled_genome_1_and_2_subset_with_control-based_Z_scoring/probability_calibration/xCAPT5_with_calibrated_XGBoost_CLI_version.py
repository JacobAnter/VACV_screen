...
parser.add_argument("XGBoost_checkpoint", type=str)
parser.add_argument("platt_checkpoint", type=str)
...
# probabilities
calibrated_probs = platt.predict_proba(logits.reshape(-1, 1))
...
# The `predict_proba` method returns one probability for each class; in
# our case, there are two classes, i.e. class 0 for no interaction and
# class 1 for interaction
...
results_df = pd.DataFrame(
   data={
      ...
      "interaction_probability": calibrated_probs[:, 1]
   }
)

results_df.to_csv(
   f"xCAPT5_interaction_probs_{args.output}_with_calibrated_XGBoost.tsv",
   sep="\t",
   index=False
)